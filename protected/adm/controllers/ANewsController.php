    ...
    class ANewsController extends AController
    {
        ...
        public function actionCreate()
        {
            ...
            if (isset($_POST['ANews'])) {
                ...
                if ($model->validate()) {
                    if (!is_dir(Yii::app()->params->upload_dir_path . Yii::app()->params->upload_news_dir)) {
                        mkdir(Yii::app()->params->upload_dir_path . Yii::app()->params->upload_news_dir, 0777, true);
                    }
                    $uploadedFile = CUploadedFile::getInstance($model, 'file');
                    if (isset($uploadedFile)) {
                        $uploads_dir        = str_replace('../', '', Yii::app()->params->upload_dir_path . Yii::app()->params->upload_news_dir);
                        $model->folder_path = $uploads_dir . $time . $uploadedFile;
                        $uploadedFile->saveAs(realpath(Yii::app()->getBasePath() . '/' . Yii::app()->params->upload_dir_path . Yii::app()->params->upload_news_dir . '/') . '/' . $time . $uploadedFile);
                    }
                    $model->created_date = date('Y-m-d H:i:s', time());
                    $model->created_by   = Yii::app()->user->username;
                    if (empty($_POST['ANews']['public_date'])) {
                        $model->public_date = date('Y-m-d H:i:s', time());
                    } else {
                        $model->public_date = date('Y-m-d H:i:s', strtotime($_POST['ANews']['public_date']));
                    }

                    if ($model->save()) {
                        /*save all tags*/
                        if ($arr_tags_form) {
                            foreach ($arr_tags_form as $tag_name) {
                                $tags = new ATags();
                                //check tags exists
                                if (!in_array($tag_name, $model_tags)) {
                                    $tags->name = $tag_name;
                                    $tags->slug = Utils::unsign_string($tags->name);
                                    if ($tags->save()) {
                                        $news_tags          = new ANewsTags();
                                        $news_tags->news_id = $model->id;
                                        $news_tags->tags_id = $tags->id;
                                        $news_tags->save();
                                    }
                                } else { /*insert to table news_tags if tags exist*/
                                    $news_tags = new ANewsTags();
                                    $tags_id   = $tags->getTagsIdByTagsName($tag_name);
                                    /*check Duplicate */
                                    if (!$news_tags->checkNewsTagsExists($model->id, $tags_id)) {
                                        $news_tags->news_id = $model->id;
                                        $news_tags->tags_id = $tags_id;
                                        $news_tags->save();
                                    }
                                }
                            }
        ...
        public function actionUpdate($id)
        {
            ...
            if (isset($_POST['ANews'])) {
                    ...
                    if ($model->save()) {
                        ...
                        if ($arr_tags_form) {
                            foreach ($arr_tags_form as $tag_name) {
                                ...
                                if (!in_array($tag_name, $model_tags)) {
                                    $tags->name = $tag_name;
                                    $tags->slug = Utils::unsign_string($tag_name);
                                    if ($tags->save()) {
                                        $news_tags          = new ANewsTags();
                                        $news_tags->news_id = $model->id;
                                        $news_tags->tags_id = $tags->id;
                        ...
                        foreach ((array)$data_lang as $k => $v) {
                            $lang_detail = ALanguages::model()->find("code_name='$k'");
                            $item = ANewsLang::model()->find("parent_id=$model->id and language_id=$lang_detail->id");
                            if(!$item){
                                $item              = new ANewsLang();
                            }
                            Languages::copyAllAttributes($item,$model);
                ...
                $data_lang_detail = ANewsLang::model()->findAll('parent_id=' . $model->id);
