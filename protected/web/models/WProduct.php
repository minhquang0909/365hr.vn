<?php
    ...
    class WProduct extends Product
    {
        /**
        ...
        public function relations()
        {
            ...
            return array(
                ...
                'lang_detail' => array(self::HAS_MANY, 'ProductLang', 'parent_id'),
            );
        }
        ...
        {
            $results    = array();
            $categories = WProductCategories::GetCategoriesInHomePage();
            if ($categories) {
        ...
        public function createUrl($id = null, $alias = null)
        {
            ...
            $str = Yii::app()->createUrl('product/detail', array('id' => $id, 'alias' => $alias));
            return $str;
        }
        ...
        public static function getListHotNews($limit = 4, $offset = 0)
        {
            ...
            $results          = WProduct::model()->findAll($criteria);
            return $results;
        }
         ...
         * @return array|mixed|null
         */
        public function getNewsRelated($news_id, $categories_id, $public_date, $limit = 5, $offset = 0)
        {
            $criteria           = new CDbCriteria;
            $criteria->distinct = true;
            $criteria->addCondition('status = ' . self::NEWS_ACTIVE);
        ...
        public function getDetailLang(){
            $data = ProductLang::model()->find('language_id='.Yii::app()->session['language_id'] .' AND parent_id='.$this->id);
            return $data;
        }
        public function getGadgetList(){

            $key_cache = 'gadget_'.Yii::app()->language;
            $cacheData = Yii::app()->cache->get($key_cache);
            if (!$cacheData) {
                if($this->created_by){
                    $cacheData = ProductGadgetLang::model()->findAll('language_id='.Yii::app()->session['language_id']);
                }
        ...
        public function getCheckedGadgetList(){
            $data = array();
            if($this->created_by){
            $data = ProductGadgetLang::model()->findAll('language_id='.Yii::app()->session['language_id'] .' AND category_id in ('.$this->created_by.')');
            }
            return $data;
        }
    }
