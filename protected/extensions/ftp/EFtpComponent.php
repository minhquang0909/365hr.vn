 ...
 * @editor        Nguyen Pham <phamvannguyen.haui@gmail.com>
 */
class EFtpComponent extends CApplicationComponent
{
     ...
     * @return    boolean
     */
    //public function get($local, $remote, $mode = FTP)
    ...
    public function upload($remote_path, $local_file, $file_name, $mode = FTP_BINARY)
    {
        if ($this->getActive()) {
            //create path
            $this->mkdir_recusive($remote_path);
            // Upload the local file to the remote location specified
            $remote = $remote_path.$file_name;
            return $this->put($remote,$local_file,$mode);
        } else {
            throw new CDbException('EFtpComponent is inactive and cannot perform any FTP operations.');
        }
    }
    ...
    {
        if ($this->getActive()) {
            $pushd = ftp_pwd($this->_connection);

            if ($pushd !== false && @ftp_chdir($this->_connection, $dir)) {
                ftp_chdir($this->_connection, $pushd);
                return true;
            }
            return false;
        } else {
    ...
    function mkdir_recusive($path)
    {
        //$path = 'video_files/2015/12/23/';
        if ($this->getActive()) {
            $parts = explode("/", $path);
            $total = count($parts);
            if(is_array($parts) && $total > 0){
                $total = count($parts);
                $current = 0;
                $part = '';
                while($current < $total){
    ...
    function get_file_size($file)
    {
        ...
        if ($this->getActive()) {
            ...
            $res = ftp_size($this->_connection, $file);
            if ($res != -1) {
                return $res;
            } else {
                return 0;
