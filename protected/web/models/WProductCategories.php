<?php
    ...
    class WProductCategories extends ProductCategories
    {
        public $sub;
        ...
        public function createUrl($id = null, $alias = null)
        {
            ...
            $str = Yii::app()->createUrl('product/categories', array('id' => $id, 'alias' => $alias));
            return $str;
        }
        ...
        {
            $key_cache = 'product_cat_menu_' . Yii::app()->session['language_id'];
            $cacheData = Yii::app()->cache->get($key_cache);
            if (!$cacheData) {
                $cate = self::model()->findAll("status=1 order by sort_order asc");
                foreach ($cate as $row) {
                    $sub      = ProductCategoriesLang::model()->find(array(
                        'select'    => 'name,slug',
