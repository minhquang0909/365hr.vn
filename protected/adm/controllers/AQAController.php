...
class AQAController extends AController
{
	/**
		...
		if(isset($_POST['AQA']))
		{
            ...
            if ($model->validate()) {
                ...
                if ($model->save()) {
                    ...
                    foreach ((array)$data_lang as $k => $v) {
                        $lang_detail = ALanguages::model()->find("code_name='$k'");
                        $item = AQALang::model()->find("parent_id=$model->id and language_id=$lang_detail->id");
                        if(!$item){
                            $item              = new AQALang();
                        }
                        Languages::copyAllAttributes($item,$model);
            ...
            /*Load Data Lang*/
            $array_field      = array('question', 'answer');
            $data_lang_detail = AQALang::model()->findAll('parent_id=' . $model->id);
	...
	{
		$model=new AQA('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['AQA'])) {
            $model->attributes = $_GET['AQA'];
        }
